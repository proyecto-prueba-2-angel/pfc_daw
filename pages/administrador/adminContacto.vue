  ...
  <div>
    ...
    <div class="admin-container mx-auto">
      ...
      <div class="bulk-actions flex flex-row space-x-8 ">
        <button :disabled="selectedContacts.length === 0" @click="bulkMoveToTrash">Descartar Seleccionados <font-awesome-icon icon="ban" /> </button>
        <router-link to="/administrador/adminTrash" class="trash-link">Papelera <font-awesome-icon icon="trash-can" /></router-link>
      </div>
      <div v-for="contact in contacts" :key="contact.id" class="contact-card">
        <div class="contact-details">
          ...
          <div class="buttons">
            <button @click="selectContact(contact)">Responder <font-awesome-icon icon="reply-all" /></button>
            <button @click="moveToTrash(contact.id)">Descartar <font-awesome-icon icon="trash" /></button>
          </div>
        </div>
      </div>
      ...
      <div v-if="selectedContact" class="modal">
        <div class="modal-content">
          ...
          <h2>Responder a {{ selectedContact.nombre }} {{ selectedContact.apellido }}</h2>
...
export default {
  ...
  methods: {
    ...
    sendResponse() {
      ...
      fetch('http://localhost/PFC/enviar_contacto.php', {
        method: 'POST',
        body: formData
      })
        .then(response => response.json())
        .then((data) => {
          ...
          if(data.success) {
            ...
            this.fetchContacts(); // Actualizar la lista de contactos después de enviar la respuesta
          } else {
    ...
    moveToTrash(id) {
      fetch('http://localhost/PFC/delete_contact.php', {
        ...
        .then((data) => {
          if(data.success) {
            alert('Solicitud movida a la papelera');
            this.fetchContacts();
          } else {
            alert('Error al mover la solicitud a la papelera: ' + data.message);
          }
        })
        .catch((error) => {
          console.error('Error al mover la solicitud a la papelera:', error);
        });
    },
    bulkMoveToTrash() {
      fetch('http://localhost/PFC/bulk_trash_contact.php', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
        ...
        .then((data) => {
          if(data.success) {
            alert('Solicitudes movidas a la papelera');
            this.selectedContacts = [];
            this.fetchContacts();
          } else {
            alert('Error al mover las solicitudes a la papelera: ' + data.message);
          }
        })
        .catch((error) => {
          console.error('Error al mover las solicitudes a la papelera:', error);
        });
    },
    prevPage() {
